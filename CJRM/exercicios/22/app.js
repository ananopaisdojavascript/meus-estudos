...
const orderedNumbers = numbers.map((number) => number).sort((a, b) => a - b);
console.log(numbers);
...
const invertOrderPeople = people.map(name => name).sort().reverse();
console.log(people)
...
const cookedIngredients = ingredients.reduce((accumulator, item, index, array) => {
    const correctWordGender = /a$/.test(item) ? "cozida" : "cozido" 
    if(index ===  array.length -1) {
        return accumulator + `${item} ${correctWordGender} `
    } else {
        return accumulator + `${item} ${correctWordGender}, ` 
    }
}, '')
...
const disneyMovies = topBrazilmovies
  ...
  .reduce((accumulator, amount) => accumulator + amount.peopleAmount, 0);
