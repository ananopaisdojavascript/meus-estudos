...
for (let i = 0; i < numbers.length; i++) {
    ...
    const isTheNumberBetween11And90 = number >= 11 && number <= 90
    if(isTheNumberBetween11And90) {
...
for(let i = 0; i < crazyArray.length; i++) {
    ...
    const isThisItemABoolean = typeOfItem === 'boolean'
    const isThisItemAString = typeOfItem === 'string'
    if(isThisItemABoolean) {
        crazyBooleans.push(crazyArray[i])
    } else if(isThisItemAString) {
        crazyStrings.push(crazyArray[i])
    } else {
        crazyNumbers.push(crazyArray[i])
    }
}

console.log(`O crazyArray tem ${crazyBooleans.length} booleans, ${crazyNumbers.length} números e ${crazyStrings.length} strings.`)
...
for(let i = 0; i < randomNumbers.length; i++) {
    ...
    const isThisNumberEven = random % 2 === 0
    if(isThisNumberEven) {
        evenNumbers.push(random)
    } else {
        oddNumbers.push(random)
    }
}



console.log(`Numeros ímpares: ${oddNumbers.slice(0, -2)}, ${oddNumbers.slice(1, -1)} e ${oddNumbers.slice(2)}. Números pares: ${evenNumbers.slice(0, -2)}, ${evenNumbers.slice(1, -1)} e ${evenNumbers.slice(2)}.`)